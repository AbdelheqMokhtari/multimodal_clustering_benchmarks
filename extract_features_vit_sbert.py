"""
extract_features_vit_sbert.py
=============================
Extract **ViT-B/16** image features and **Sentence-BERT** text features
from a raw multimodal dataset and save them to a .mat file.
...
- Text  : ``all-MiniLM-L6-v2`` from **sentence-transformers**
          → 384-dimensional sentence embedding (fast & accurate).

Usage
-----
    python extract_features_vit_sbert.py --dataset coco_cross
    python extract_features_vit_sbert.py --dataset coco_all
    python extract_features_vit_sbert.py --dataset pascal
    python extract_features_vit_sbert.py --dataset coco_cross coco_all pascal

Output
------
For each dataset a ``<dataset>/<dataset>_vit_sbert.mat`` file is created with:
...
def process_dataset(
    ...
    output_mat = os.path.join(base_dir, f"{dataset_name}_vit_sbert.mat")
    ...
    print(f"     X2 (txt): {X2.shape}  [Sentence-BERT]")
    print(f"     Y       : {Y.shape}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract ViT-B/16 + Sentence-BERT features for multimodal datasets"
    )
    parser.add_argument(
        "--dataset",
        nargs="+",
    ...
    vit_model = timm.create_model(
        "vit_base_patch16_224", pretrained=True, num_classes=0  # num_classes=0 → feature extractor
    )
    vit_model = vit_model.to(device).eval()

    # Build the standard transform for this model
